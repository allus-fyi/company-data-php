...
|------------------|----------|----------|---------|---------|
| `api_url` | `apiUrl` | yes | — | API base, e.g. `https://api.allme.fyi` — the platform's global front door. The token response names your home region and the SDK sends its data calls there from then on (README, **How it's wired** → Regions). |
